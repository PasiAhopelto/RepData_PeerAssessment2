...
```{r}
fatalitiesSumPlot <- ggplot(topFatalitiesSumByEventType[,c('EVTYPE', 'FATALITIES')], aes(x=EVTYPE, y=FATALITIES)) + geom_bar(stat="identity") + coord_flip() + ylab('Fatalities (sum)') + xlab('')
injuriesSumPlot <- ggplot(topInjuriesSumByEventType[,c('EVTYPE', 'INJURIES')], aes(x=EVTYPE, y=INJURIES)) + geom_bar(stat="identity")  + coord_flip() + ylab('Injuries (sum)') + xlab('')
...
```{r}
fatalitiesMeanPlot <- ggplot(topFatalitiesMeanByEventType[,c('EVTYPE', 'FATALITIES')], aes(x=EVTYPE, y=FATALITIES)) + geom_bar(stat="identity") + coord_flip() + ylab('Fatalities (mean)') + xlab('')
injuriesMeanPlot <- ggplot(topInjuriesMeanByEventType[,c('EVTYPE', 'INJURIES')], aes(x=EVTYPE, y=INJURIES)) + geom_bar(stat="identity")  + coord_flip() + ylab('Injuries (mean)') + xlab('')
layout <- matrix(seq(1, 2), ncol = 1, nrow = 2)
grid.newpage()
pushViewport(viewport(layout = grid.layout(nrow(layout), ncol(layout))))
...
```{r}
damagesSumPlot <- ggplot(topDamagesSumByEventType[,c('EVTYPE', 'SUM')], aes(x=EVTYPE, y=SUM)) + geom_bar(stat="identity") + coord_flip() + ylab('Damages (sum)') + xlab('')
damagesMeanPlot <- ggplot(topDamagesMeanByEventType[,c('EVTYPE', 'MEAN')], aes(x=EVTYPE, y=MEAN)) + geom_bar(stat="identity") + coord_flip() + ylab('Damages (mean)') + xlab('')
layout <- matrix(seq(1, 2), ncol = 1, nrow = 2)
grid.newpage()
pushViewport(viewport(layout = grid.layout(nrow(layout), ncol(layout))))
