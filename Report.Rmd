...
head(topFatalitiesMeanByEventType)
head(topInjuriesSumByEventType)
head(topInjuriesMeanByEventType)
```
...
```{r}
fatalitiesSumPlot <- ggplot(topFatalitiesSumByEventType[,c('EVTYPE', 'FATALITIES')], aes(x=EVTYPE, y=FATALITIES)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
injuriesSumPlot <- ggplot(topInjuriesSumByEventType[,c('EVTYPE', 'INJURIES')], aes(x=EVTYPE, y=INJURIES)) + geom_bar(stat="identity")  + theme(axis.text.x = element_text(angle = 90, hjust = 1))
